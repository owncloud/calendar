OC.L10N.register(
    "calendar",
    {
    "Calendar" : "Egutegia",
    "%s has published the calendar \"%s\"" : "%s-k \"%s\" egutegia argitaratu du",
    "Too many redirects. Aborting ..." : "Berbideratze gehiegi. Abortatzen...",
    "The remote server did not give us access to the calendar (HTTP {%s} error)" : "Urrutiko zerbitzariak ez digu egutegira sarbiderik eman (HTTP {%s} errorea)",
    "Error connecting to remote server" : "Errorea urrutiko zerbitzarira konektatzen",
    "Error requesting resource on remote server. This could possibly be related to a certificate mismatch" : "Errorea urrutiko zerbitzariari baliabide bat eskatzean. Errorea egiaztagiri baten  desadostasunarekin egon liteke.",
    "Error requesting resource on remote server" : "Errorea urrutiko zerbizatziaren baliabidea eskatzen",
    "Individual" : "Banakakoa",
    "Group" : "Taldea",
    "Resource" : "Baliabidea",
    "Room" : "Gela",
    "Unknown" : "Ezezaguna",
    "Required" : "Derrigorrezkoa",
    "Optional" : "Hautazkoa",
    "Does not attend" : "Ez da joango",
    "{name} ({email})" : "{name} ({email})",
    "You are in an unknown timezone ({tz}), falling back to UTC" : "Ordu zonalde ezezagunean zaude ({tz}), UTC-ra itzutzen",
    "Please create a calendar first." : "Egutegia sortu lehenago mesedez.",
    "New event" : "Gertaera berria",
    "CalDAV address for clients" : "Bezeroentzako CalDAV helbidea",
    "Error saving WebCal-calendar" : "Errorea WebCal-calendarra gordetzen",
    "WebDAV address for subscriptions" : "WebDAV helbidea harpidetzentzat",
    "Email has been sent." : "Posta elektronikoa bidali da.",
    "There was an issue while sending your email." : "Arazo bat egon da zure posta elektronikoa bidaltzerakoan.",
    "group" : "taldea",
    "Details" : "Xehetasunak",
    "Attendees" : "Partaideak",
    "Reminders" : "Abisuak",
    "Repeating" : "Errepikapena",
    "When shared show full event" : "Partekatuta badago erakutsi gertaera osoa",
    "When shared show only busy" : "Partekatuta badago lanpetuta bakarrik erakutsi",
    "When shared hide this event" : "Partekatuta badago ezkutatu gertaera hau",
    "Confirmed" : "Baieztatuta",
    "Tentative" : "Behin behinekoa",
    "Cancelled" : "Baztertua",
    "Please add a title." : "Mesedez, izenburu bat gehitu.",
    "Please select a calendar." : "Mesedez, hautatu egutegi bat",
    "The event can not end before it starts." : "Gertaera ezin da bukatu hasi baino lehenago.",
    "Global" : "Globala",
    "None" : "Ezer",
    "Never" : "Inoiz",
    "Daily" : "Egunero",
    "Weekly on {weekday}" : "Astero {weekday} egunean",
    "Monthly on {day}" : "Hilabetero {day} egunean",
    "Yearly on {day} of {month}" : "Urtero {month} hilabetearen {day} egunean",
    "Custom" : "Pertsonalizatua",
    "Day(s)" : "Eguna(k)",
    "Week(s)" : "Astea(k)",
    "Month(s)" : "Hilabetak(k)",
    "Year(s)" : "Urtea(k)",
    "First" : "Lehenengoa",
    "Second" : "Bigarrena",
    "Third" : "Hirugarrena",
    "Fourth" : "Laugarrena",
    "Last" : "Azkena",
    "How about getting started by importing some calendars?" : "Zer iruditzen zaizu egutegi batzuk inportatuz hasten bagara?",
    "At time of event" : "Gertaeraren unean",
    "5 minutes before" : "5 minutu lehenago",
    "10 minutes before" : "10 minutu lehenago",
    "15 minutes before" : "15 minutu lehenago",
    "30 minutes before" : "30 minutu lehenago",
    "1 hour before" : "ordu 1 lehenago",
    "2 hours before" : "2 ordu lehenago",
    "Audio" : "Audio",
    "Email" : "E-posta",
    "Pop-up" : "Popup",
    "sec" : "seg",
    "min" : "min",
    "hours" : "ordu",
    "days" : "egun",
    "week" : "aste",
    "before" : "aurretik",
    "after" : "ondoren",
    "start" : "hasi",
    "end" : "bukaera",
    "Editing reminders of unknown type not supported." : "Mota ezezaguneko oroigarrien ediziorako euskarrik ez.",
    "Hour" : "Ordua",
    "Minute" : "Minutu",
    "Now" : "Orain",
    "Week {number} of {year}" : "{year}ko {number}. astea",
    "Successfully imported" : "Ongi inportatu da",
    "Partially imported, 1 failure" : "Partzialki inportatuta, huts egite 1",
    "Partially imported, {n} failures" : "Partzialki inportatuta, {n} huts egite",
    "Audio alarm" : "Audio alarma",
    "{type} {time} before the event starts" : "{time} {type} gertaera hasi arte",
    "{type} {time} before the event ends" : "{time} {type} gertaera bukatu arte",
    "{type} {time} after the event starts" : " {time} {type} gertaera hasi ondoren",
    "{type} {time} after the event ends" : " {time} {type} gertaera bukatu ondoren",
    "{type} at the event's start" : "{type} gertaeraren hasieran",
    "{type} at the event's end" : "{type} gertaeraren amaieran",
    "{type} at {time}" : "{type} {time}-en",
    "{calendar} shared by {owner}" : "{calendar}  {owner}-(e)k partekatua",
    "Please enter a valid WebCal-URL" : "Sartu mesedez WebCal-URL egokia",
    "Severe error in webcal proxy. Please contact administrator for more information." : "Errore larria webcal proxyan. Mesedez, harremanetan jarri kudeatzailearekin informazio gehiagorako.",
    "Title of the Event" : "Gertaeraren izenburua",
    "Starts" : "Hasierak",
    "Ends" : "Bukaerak",
    "All day Event" : "Egun osoko gertaera",
    "Delete" : "Ezabatu",
    "Delete all" : "Ezabatu dena",
    "Delete this event " : "Gertaera hau ezabatu",
    "Cancel" : "Ezeztatu",
    "More ..." : "Gehiago ...",
    "Create" : "Sortu",
    "Update" : "Eguneratu",
    "Close" : "Itxi",
    "Location" : "Kokapena",
    "Description" : "Deskribapena",
    "Event status" : "Gertaeraren egoera",
    "Visibility when sharing" : "Ikuspena partekatzean",
    "Export" : "Esportatu",
    "%s has published the calendar %s" : "%sk %s egutegia argitaratu du",
    "Hello," : "Kaixo,",
    "We wanted to inform you that %s has published the calendar {boldstart}%s{boldend}." : "Jakinarazi nahi dizugu %s-ek {boldstart}%s{boldend} egutegia argitaratu duela.",
    "Click here to access it" : "Klikatu hemen sarbidea izateko",
    "Cheers!" : "Ongi izan!",
    "We wanted to inform you that %s has published the calendar %s." : "Jakinarazi nahi dizugu %s-ek %s egutegia argitaratu duela.",
    "Click on the link below to access it" : "Klikatu beheko lotura sarbidea izateko",
    "Due to an upstream bug, the calendar is currently not able to provide support for the asset pipeline feature." : "Upstream-ek duen akats baten ondorioz, egutegiak momentuz ez du euskarririk asset pipeline ezaugarriarentzat.",
    "Please contact your administrator and ask them to disable the \"asset-pipeline.enabled\" config option." : "Mesedez, harremanetan jarri zure kudeatzailearekin eta eskatu iezaiozu \"asset-pipeline.enabled\" konfigurazio aukera ezgaitzeko.",
    "Day" : "Eguna",
    "Week" : "Astea",
    "Month" : "Hilabetea",
    "Today" : "Gaur",
    "Some events in this calendar are broken. Please check the JS console for more info." : "Egutegi honetako hainbat gertaera hautsita daude. Informazio gehiagorako begiratu JS kontsola.",
    "Share Calendar" : "Partekatu Egutegia",
    "Shared" : "Partekatua",
    "More" : "Gehiago",
    "Edit" : "Editatu",
    "Link" : "Esteka",
    "iCal link" : "iCal lotura",
    "Download" : "Deskargatu",
    "Share with users or groups" : "Partekatu erabiltzaile edo taldeekin.",
    "can edit" : "editatu dezake",
    "Share link" : "Partekatu esteka",
    "Public access" : "Sarbide publikoa",
    "Email link to person" : "Lotura posta bidez bidali pertsonai",
    "Send" : "Bidali",
    "New Calendar" : "Egutegi berria",
    "Name" : "Izena",
    "New Subscription" : "Harpidetza Berria",
    "Go back" : "Atzera",
    "Go forward" : "Aurrera joan",
    "Calendar does not exist" : "Egutegia ez da existitzen",
    "Maybe you got a wrong link or the calendar was unshared?" : "Agian lotura okerra jaso duzu edo egutegia ez zegoen partekatuta?",
    "Edit event" : "Gertaera editatu",
    "Delete Event" : "Gertaera ezabatu",
    "Save Event" : "Gertaera gorde",
    "Time" : "Noiz",
    "Type" : "Mota",
    "Relative" : "Erlatiboa",
    "Absolute" : "Absolutua",
    "Repeat" : "Errepikatu",
    "times every" : "aldiz",
    "Add" : "Gehitu",
    "Email address of attendee" : "Partaidearen posta helbidea",
    "Please add your email address in the personal settings in order to add attendees." : "Partaideak gehitu ahal izateko, gehitu ezarpen pertsonaletan zure posta helbidea.",
    "from" : "Noiztik",
    "to" : "Noiz arte",
    "This event's repeating rule is not supported yet." : "Gertaera honen errepikatzeko araua oraindik ez dago onartuta.",
    "Support for advanced rules will be added with subsequent updates." : "Arau aurreratuen euskarria hurrengo eguneraketetan gehituko da.",
    "Reset repeating rule" : "Errepikatzeko araua berrasieratu",
    "Repeat every:" : "Errepikatu (every):",
    "Repeat on:" : "Errepikatu (on):",
    "Repeat on every:" : "Errepikatu (on every):",
    "of each month" : "hilabete bakoitzean",
    "End:" : "Amaiera:",
    "never" : "inoiz",
    "event(s)" : "gertaera(k)",
    "on" : "(on)",
    "Import Calendars" : "Inportatu egutegiak",
    "Import canceled" : "Inportazio ezeztatua",
    "Analyzing calendar" : "Egutegia aztertzen",
    "The file contains objects incompatible with the selected calendar" : "Hautatutako egutegiarekin bateraezinak diren objektuak dauzka fitxategiak",
    "New calendar" : "Egutegi berria",
    "File is empty" : "Fitxategia hutsik dago",
    "Import scheduled" : "Inportazio programatua",
    "%s shared the calendar <strong>%s</strong> with you" : "%s-ek <strong>%s</strong>egutegia partekatu du zurekin",
    "Embed" : "Txertatu",
    "Iframe to integrate" : "Txertatu beharrezko iframe-a",
    "Publish URL" : "Argitaratu URLa",
    "Settings & import" : "Ezarpenak eta inportazioa",
    "Skip simple event editor" : "Saltatu gertaeraren edizioa",
    "Show week numbers" : "Asteen zenbakiak erakutsi",
    "Import calendar" : "Egutegia Inportatu",
    "No Calendars selected for import" : "Ez duzu inportatzeko egutegirik aukeratu",
    "Primary CalDAV address" : "CalDAV helbide nagusia",
    "iOS/OS X CalDAV address" : "iOS/OS X CalDAV helbidea",
    "Subscribe" : "Harpidetu"
},
"nplurals=2; plural=(n != 1);");
